...
CLASS_MAP = {0: "AD", 1: "FTD", 2: "CN"}
def batched_predict(model, segments, mean, std, device, batch_size=16):
    preds = []

    mean_t = torch.tensor(mean[:, None, None]).float().to(device)
    std_t = torch.tensor(std[:, None, None]).float().to(device)

    for i in range(0, len(segments), batch_size):
        batch = segments[i:i+batch_size]

        x = torch.tensor(batch).float().to(device)
...
def predict_with_streaming_ensemble(segments, model_family):
    ...
    for idx, fname in enumerate(model_files):
        ...
        seg_preds = batched_predict(
            model,
            segments,
            mean,
            std,
            device,
            batch_size=8   # có thể giảm còn 4 nếu vẫn nặng
        )
        ...
        # ===== CLEAN =====
        del model, state_dict, seg_preds
        if device.type == "cuda":
            torch.cuda.empty_cache()
        gc.collect()
